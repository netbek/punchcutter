...
module.exports = function(config) {
  ...
  return Promise.mapSeries(config.build.rules, rule =>
    ...
    .then(function(rules) {
      const defaultRuleGlyphs = _.values(rules[0].sprite.coordinates);
      const defaultGlyph = defaultRuleGlyphs.length ? defaultRuleGlyphs[0] : {};
      ...
      const cssData = {
        ...
        glyphHeight: defaultGlyph.height,
        glyphWidth: defaultGlyph.width,
        groups: _.values(
          _.groupBy(
            rules.reduce(
              (result, rule) =>
