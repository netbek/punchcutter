...
module.exports = function(config) {
  ...
  return Promise.mapSeries(config.build.rules, rule =>
    ...
    .then(function(rules) {
      ...
      const {sprite: defaultSprite} = rules[0];
      const {
        width: spriteWidth,
        height: spriteHeight
      } = defaultSprite.properties;
      const defaultRuleGlyphs = _.values(defaultSprite.coordinates);
      const {width: glyphWidth, height: glyphHeight} = defaultRuleGlyphs.length
        ? defaultRuleGlyphs[0]
        : {};
