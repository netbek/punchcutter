...
module.exports = function(config) {
  ...
  return Promise.mapSeries(config.build.rules, rule =>
    ...
    .then(function(rules) {
      ...
      const {
        width: baselineSpriteWidth,
        height: baselineSpriteHeight
      } = baselineSprite.properties;
      const baselineRuleGlyphs = _.values(baselineSprite.coordinates);
      const {
        width: baselineGlyphWidth,
        height: baselineGlyphHeight
      } = baselineRuleGlyphs.length ? baselineRuleGlyphs[0] : {};
      ...
      // If glyph size is declared in config, then use those values and compute
      // sprite size based on them.
      if (config.build.glyphWidth && config.build.glyphHeight) {
        glyphWidth = config.build.glyphWidth;
        glyphHeight = config.build.glyphHeight;
        spriteWidth =
          baselineSpriteWidth / baselineGlyphWidth * config.build.glyphWidth;
        spriteHeight =
          baselineSpriteHeight / baselineGlyphHeight * config.build.glyphHeight;
      } else {
        glyphWidth = baselineGlyphWidth;
        glyphHeight = baselineGlyphHeight;
        spriteWidth = baselineSpriteWidth;
        spriteHeight = baselineSpriteHeight;
      }
