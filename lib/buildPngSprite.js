...
module.exports = function (config) {
  ...
  return Promise.mapSeries(config.build.rules, (rule) =>
    ...
    .then(function (rules) {
      ...
      return fs
        .readFile(path.join(__dirname, '../src/png-sprite.scss.njk'), 'utf8')
        .then((data) =>
          Promise.resolve(
            ...
            switch (stylesheet) {
              case 'css':
                return postcss([autoprefixer(config.build.autoprefixer)])
                  ...
                  .then((result) => fs.outputFile(cssPath, result.css, 'utf8'));

              case 'scss':
                return fs.outputFile(scssPath, data, 'utf8');
            }
    ...
    .then(function (rules) {
      if (config.build.htmlDemo) {
        ...
        return fs
          .readFile(path.join(__dirname, '../src/png-sprite.html.njk'), 'utf8')
          .then((data) =>
            Promise.resolve(nunjucks.renderString(data, htmlData))
          )
          .then((data) => fs.outputFile(htmlPath, data, 'utf8'));
      }
