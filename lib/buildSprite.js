...
module.exports = function(config) {
  const svgBuilds = config.build.builds.filter(
    build => build.buildType === SVG
  );
  ...
  return loadFiles(config.src)
    ...
    .then(function(files) {
      return Promise.mapSeries(svgBuilds, function(svgBuild) {
        ...
        files.forEach(function(file) {
          ...
          store.add(svgBuild.idPrefix + basename, file.data);
        });

        const data = store.toString();
        const distPath = path.resolve(svgBuild.dist, config.name + '.svg');
